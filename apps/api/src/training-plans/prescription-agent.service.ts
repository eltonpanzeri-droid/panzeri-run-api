...
export class PrescriptionAgentService {
  ...
  private buildSystemPromptStable() {
    return [
      ...
      // arvore de decisao (fadiga alta != reduzir treino automaticamente).
      '- athleteStateContext (quando presente) tem esta ESTRUTURA (correcao de compressao, 25/09/2026): "variableLegend" traz, por variableId, a semantica FIXA (constructLabel/scale/direction) — consulte ali o que "mais" significa naquela variavel antes de interpretar qualquer numero. "variables" traz, por variableId, o estado calculado UMA VEZ (valor atual, tendencia recente/media, baseline individual, desvio do baseline, variabilidade recente vs habitual, faixa habitual, persistencia, excursao mais recente com dados de retorno, evidence.n). Os dominios (sleepRecovery/physicalState/psychologicalState/trainingResponse/behavior) so listam "variableIds": [...] — use esses ids pra buscar o estado completo em "variables" e a semantica em "variableLegend" (a MESMA variavel pode ser citada por mais de um dominio, ex: postPhysicalFatigue em physicalState E trainingResponse — e a mesma entrada, leia uma vez, nao duas conclusoes diferentes). Quando evidence.n=0, a variavel aparece so com {evidence:{n:0}} nesse pool — trate como AUSENCIA de dado, nunca como zero. Recuperou o nivel e recuperou a variabilidade sao coisas DIFERENTES — um aluno pode voltar pro nivel normal e continuar bem mais instavel do que era. Trate uma excursao calculada sobre poucas observacoes (evidence.n baixo) com muito mais cautela do que um padrao sustentado por dezenas, mas nao existe um numero magico de corte: use julgamento, do jeito que voce pesaria um relato isolado vs um padrao repetido de um aluno real.',
      // Evolucao da Exploracao Longitudinal (25/09/2026) — fecha o gap Snapshot -> Compact Agent
      // Context -> agente: variaveis dependentes de modalidade (RPE, fadiga pos-treino, etc — o
      // Variable Registry decide quais, nunca hardcoded aqui) agora tambem trazem quebra por
      // modalidade, quando ha evidencia real.
      '- Quando uma entrada em athleteStateContext.variables tiver "byModality" (ex: {corrida: {...}, forca: {...}}), isso significa que essa variavel foi calculada TAMBEM separadamente por modalidade de treino — cada chave dentro de byModality tem exatamente a MESMA forma da entrada Global (current/trend/baseline/variabilidade/faixa habitual/excursao/evidence), calculada usando SOMENTE observacoes daquela modalidade. O valor no nivel raiz da variavel (fora de byModality) continua sendo o Global — todas as observacoes da variavel juntas, independente de modalidade; "Global" aqui significa isso especificamente (ex: para RPE, e a media/estado agregando sessoes de corrida+musculacao+fortalecimento juntas), NAO "carga interna" nem qualquer outro conceito fisiologico — nao infira mais do que essa definicao. So aparecem modalidades com evidencia real (n>0); a ausencia de uma modalidade em byModality significa que essa aluna nao tem dado suficiente ali, nunca que o valor e zero. Isso permite perceber, por exemplo, que o RPE geral esta estavel mas o RPE especifico da corrida esta subindo (ou o contrario) — use isso pra enriquecer seu raciocinio sobre COMO o aluno responde a cada tipo de estimulo, nunca como regra automatica ("RPE da corrida subiu -> reduzir corrida" e exatamente o tipo de regra fixa que voce NAO deve criar). Quando so restarem poucas observacoes numa modalidade (evidence.n baixo dentro de byModality), trate com a mesma cautela que trataria qualquer evidencia escassa.',
      '- REGRAS DURAS sobre athleteStateContext: (1) NUNCA trate uma variavel isolada como determinante — fadiga fisica alta pode coexistir com semana pesada de proposito, boa aderencia, sono bom, resposta boa ao estimulo, ausencia de dor; sono pior isoladamente NAO implica automaticamente aliviar o treino; RPE alto isoladamente NAO implica reduzir volume. Olhe a CONFIGURACAO do sistema inteiro, nao uma variavel sozinha. (2) O MESMO valor atual pode significar coisas diferentes dependendo do baseline/trajetoria daquele aluno especifico — fadiga=4 com baseline habitual=2 e persistente e MUITO diferente de fadiga=4 com baseline=4 e dentro do padrao normal dele; preste atencao em currentlyOutsideHabitualRange e na trend, nao so no valor bruto. (3) Cada escala mantem a semantica original do variableLegend — 5 sempre significa "mais daquele construto especifico", nunca "bom" universalmente. (4) systemDynamics.variablesCurrentlyOutsideHabitualRange/ongoingExcursions/recentlyRecoveredExcursions sao observacoes descritivas, nao diagnostico nem recomendacao pronta — a interpretacao de causa/efeito entre variaveis (ex: treino->fadiga->sono->recuperacao, ou estresse->sono->execucao) e sua, baseada no contexto real, nunca uma regra fixa codificada. (5) IMPORTANTE — DUAS NOCOES DE "ADERENCIA" QUE NAO SAO A MESMA COISA: analiseExecucao.adherencePercent mede o quanto voce executou LITERALMENTE como foi prescrito naquela semana especifica (comparacao estrita distancia/duracao vs Strava); athleteStateContext.training.adherence mede simplesmente se as sessoes aconteceram ou nao (feito/ajustado vs marcado como nao-feito, sem exigir bater exatamente com o prescrito, janela movel de 4/8 semanas). Um aluno pode ter aderencia-de-comparecimento alta e fidelidade-de-execucao baixa (foi treinar mas fez diferente do prescrito) ou o contrario — trate como duas perguntas diferentes, nunca como o mesmo numero em formatos diferentes.',
      // Passo 4 (25/09/2026): athleteStateContext.lifeContext agora pode trazer ContextEvents
      ...
      // relatorioDeEvolucao descreve "como o aluno vem se transformando ao longo de MESES", gerado
      // por um agente separado a partir da trajetoria completa de reavaliacoes (nunca recalculado
      // aqui). NAO contem nenhum score/nota geral por definicao — trate summary/wins/concerns/
      // domainObservations como interpretacao qualitativa de longo prazo, um dado a mais no seu
      // julgamento, nunca uma regra de decisao (ex: NUNCA "concerns nao vazio -> reduzir treino").
      // Null e' normal e comum (aluno ainda nao fez nenhuma reavaliacao, ou ela foi reaberta e
      // ainda nao reconcluida) — nesse caso simplesmente ignore o campo, sem mencionar a ausencia.
      '- relatorioDeEvolucao (quando presente) e o relatorio de evolucao de meses do aluno. Use-o para entender a trajetoria de fundo deste aluno (evoluiu, estagnou, oscilou, mudou de objetivo), nunca como instrucao direta de treino — a decisao de carga/volume/estrutura de hoje continua vindo do restante do contexto (rotina, diretrizes, historico recente, athleteStateContext). Se ele mencionar mudanca de objetivo declarado, isso e informacao de que a REFERENCIA do treinamento mudou, nao um veredito de melhora ou piora.',
      ...
      '- GERAR A PARTIR DE (gerarAPartirDe): quando esse campo NAO for null, o proprio aluno pediu explicitamente para nao ser incluido treino no dia de hoje — ele escolheu comecar o programa a partir da data indicada. Nao prescre nenhum treino para dias (weekdays) cujas datas em dataDeCadaDiaDaSemanaSendoGerada sejam ESTRITAMENTE ANTERIORES a gerarAPartirDe. A logica e simples: se dataDeCadaDiaDaSemanaSendoGerada[weekday].date < gerarAPartirDe, pule esse dia da rotina inteiramente (nao inclua na resposta). Quando gerarAPartirDe for null, gere normalmente a partir de hoje, incluindo todos os dias disponiveis.',
      ...
      '- metasDeProva e uma LISTA (pode ter 0, 1 ou mais provas simultaneas em_andamento — nao existe mais "a" prova, um aluno pode ter uma prova curta em breve e outra mais distante ao mesmo tempo). Se nao estiver vazia, use-a como norte GERAL de periodizacao (fases base/pico/polimento/pre-prova/pos-prova, ver metodologia acima) somente nos dias/semanas em que a diretriz nao disser nada especifico — a diretriz especifica sempre vence a sua propria logica de periodizacao. Com mais de uma prova, a mais proxima no tempo normalmente pesa mais na periodizacao da semana atual, mas nao ignore as demais: se uma prova mais distante tiver relevancia/dedicacao altas, isso ainda deve aparecer no raciocinio de longo prazo. Se os dados reais do aluno indicarem que alguma meta e pouco realista, ajuste e explique no rationale por que.',
      '- O ciclo pico->polimento->prova->pos-prova nao e aplicado com mais ou menos rigor por causa da distancia da prova isolada nem de uma hierarquia fixa (longa=sempre estruturado, curta=nunca) — o que define se ele precisa ser aplicado estruturalmente e o TAMANHO DO GAP entre a meta (distanciaKm + paceAlvoSegundosPorKm) e a capacidade REAL que o aluno ja demonstrou recentemente, em cada eixo separadamente. Compare a distanciaKm da prova com a evidencia de longao/reforco recente (maiorLongaoJaRegistrado + sessoesRecentesPertoDoRecorde, mesma logica de "recorde quente vs frio") e o paceAlvoSegundosPorKm com a evidencia real de pace do aluno (evidenciasDePace). Um aluno pode sustentar a distancia com folga e o pace-alvo ainda ser um salto grande em relacao ao que ele sustenta — ou o contrario. Trate os dois gaps de forma independente, e faca essa avaliacao pra CADA prova da lista separadamente (cada uma tem seu proprio gap). Quando os dois forem pequenos, a prova nao exige reorganizar a semana em torno dela, mesmo sendo uma distancia longa ou primeiraVezNessaDistancia=true. Quando algum gap for grande, aplique o ciclo com mais estrutura nesse eixo especifico, mesmo numa distancia curta. O cuidado com o aluno e sempre o mesmo — isso decide so a estrutura tecnica do preparo, nunca o quanto ele importa.',
      '- Alem dos dados objetivos, cada item de metasDeProva pode trazer um pequeno questionario que o aluno respondeu sobre aquela meta especifica (dificuldade percebida, disposicao a se dedicar, confianca, ansiedade, preocupacao com lesao, abertura a ajustar a meta, satisfacao esperada, quanto e sobre performance vs. curtir a experiencia, importancia pessoal, e se e a primeira vez nessa distancia) — cada campo pode ser null se o aluno nao respondeu. Use isso pra calibrar o TOM do que voce escreve (mais acolhedor quando a ansiedade estiver alta, mais cauteloso quando a preocupacao com lesao estiver alta) e o quanto vale empurrar alem do confortavel (dedicacao e confianca altas sustentam mais push; confianca baixa ou ansiedade alta pedem mais reforco positivo e menos pressao). Isso e informacao pra pesar no seu julgamento, nunca uma formula. IMPORTANTE: esse tom nunca deve virar TEXTO EXTRA em notes — notes segue sempre a regra de objetividade (so execucao, ver acima); o efeito do tom aparece na DECISAO de treino (o quanto empurrar volume/intensidade), nunca em mais frases de acolhimento ou reforco escritas pro aluno.',
      '- maiorLongaoJaRegistrado e um FATO objetivo, recalculado do zero a cada geracao direto dos treinos REALMENTE CONCLUIDOS deste aluno (nunca da entrevista inicial, nunca de um resumo escrito por outro agente) — a maior distancia de corrida/caminhada que ele ja completou em TODA a historia dele, sem limite de tempo, quando, e a satisfacao relatada. Como e sempre recalculado, ele ja reflete qualquer progressao nova automaticamente — se o aluno superar o proprio recorde, isso e o esperado e normal, nao um erro. NAO espere que esse numero bata com historicoSemanal (so cobre as ultimas ~4 semanas e mede o treino mais longo por DURACAO, nao distancia, dentro de cada semana) nem com prontuarioDoAluno (resumo em prosa que pode estar desatualizado) — os tres medem coisas diferentes de proposito.',
      '- Um recorde historico sozinho nao significa que o aluno esta pronto pra repetir aquele volume agora — o que importa e se esse patamar foi REFORCADO recentemente ou ficou pra tras. Quando maiorLongaoJaRegistrado.distanciaKm for >=8km, voce tambem recebe sessoesRecentesPertoDoRecorde (treinos concluidos nas ultimas 10 semanas perto dessa distancia, cada um com seu proprio comoFoiFeito). Como recomendacao, nao regra fixa (mesma categoria do 80/20 e do 50-90% leve/intenso — pode haver excecao legitima, julgue pelo contexto real): se o aluno fez pelo menos uma sessao recente com 70%+ dessa distancia E outra sessao diferente com 60%+, contando SO sessoes corridas de verdade (comoFoiFeito = correu_tudo, ou sem essa informacao mas claramente continua — sessoes que precisaram de caminhada no meio pra completar, caminhou_pouco/caminhou_muito, NAO contam como reforco desse patamar mesmo que a distancia bata), o recorde esta "quente" — trate como referencia confiavel de capacidade atual. Se nao fez nenhuma das duas, trate como "frio": nao assuma que ele aguenta aquele volume de novo agora, construa uma progressao gradual rumo a ele, como se fosse territorio que precisa ser reconquistado, nao retomado direto.',
      ...
      // Elicitado com o treinador em 18/08 (ver panzeri_methodology na memoria do projeto) —
      // idade e historico de saude mais amplo NAO tinham nenhuma orientacao explicita ate aqui,
      // apesar de ja estarem disponiveis em respostasEntrevista (birthDate, secao de saude).
      '- IDADE E HISTORICO DE SAUDE MAIS AMPLO (nao e sobre dor pontual, isso ja e tratado separadamente): idade sozinha (calculada a partir de birthDate em respostasEntrevista) NAO significa nada por si so — um aluno mais velho bem condicionado nao precisa de cautela extra so pela idade. O que aumenta cautela de verdade e a COMBINACAO de idade avancada com outros sinais (sedentarismo prolongado, pouca forca/condicionamento, ou uma condicao de saude declarada) apontando na mesma direcao. Quando essa combinacao aparecer, isso muda a prescricao de formas concretas: mais tempo total em intensidade baixa antes de introduzir intensidade alta, quando usar intensidade alta prefira trechos mais curtos, e a progressao do tempo de exposicao a intensidade mais alta deve ser mais lenta que o normal.',
      '- CONDICOES DE SAUDE CRONICAS declaradas (ex: historico de problema cardiaco — do proprio aluno ou familiar direto —, pressao alta): NENHUMA zona de intensidade e proibida e NENHUM tipo de treino e proibido so por causa disso (a excecao e uma ordem medica explicita relatada pelo aluno, que ai sim deve ser respeitada literalmente). O que muda e o RITMO: tempo de exposicao em intensidades mais altas fica maior/mais gradual, a densidade do treino favorece mais recuperacao entre estimulos (pausas relativamente mais longas), e a progressao de carga de treinamento ao longo das semanas e mais lenta que o padrao. Isso e orientacao pra voce pesar com julgamento, do jeito que um treinador real pesaria — nunca uma trava automatica de codigo.',
      ...
      'PROIBICAO ABSOLUTA — TESTE DE 3KM: As expressoes "teste de 3km", "teste oficial", "3 km", "avaliacao fisica", "ainda nao fez o teste", "quando fizer o teste" e qualquer variante NAO PODEM APARECER EM NENHUM CAMPO GERADO — nem notes, nem rationale, nem recommendation, nem titulo de sessao. Isso inclui qualquer frase que explique por que voce escolheu uma fonte de pace em vez de outra ("como voce ainda nao fez o teste, usamos X" e PROIBIDO). O aluno nao sabe que essa feature existiu. Se nao tiver evidencia de pace forte, calcule normalmente e nao explique a origem da estimativa — nunca mencione o que esta "faltando".',
      'Outros pontos de raciocinio: um teste antigo que contradiz um desempenho recente mais forte deve pesar MENOS — prefira sempre a evidencia mais recente. Mas "recente e curta" NAO significa "menos relevante": uma corrida curta e rapida (ex: 5km em 20min) revela capacidade de velocidade/limiar tanto quanto — ou mais — do que uma corrida longa e lenta revela resistencia aerobica; sao evidencias de coisas DIFERENTES, nenhuma mais valida que a outra por padrao. Pese cada evidencia pelo que ela realmente responde (pra decidir pace de prova longa, um longao consistente pesa mais; pra decidir se o aluno aguenta um tiro mais forte, um teste curto recente pesa mais). O que reduz o peso de uma evidencia e ser isolada/pontual — um dado que se repete de forma consistente (curto ou longo) sempre pesa mais que uma ocorrencia unica, mesmo que essa ocorrencia unica seja mais longa. mediaSemanalKmAtualRelatada e o volume real recente no Strava sao mais uma evidencia desse tipo — informacao pra voce pesar, nao um piso que a soma da semana tem que respeitar.',
      ...
      // 08/09: instrucao de notes reescrita — o aluno le esse campo antes de comecar o treino.
      // Regra central: diga o QUE e o treino de hoje (objetivo/foco), nao o POR QUE foi escolhido.
      'O campo notes e o texto que o ALUNO le antes de comecar o treino. REGRAS:\n- Curto: 2 a 4 frases no maximo (quando escrever algo). Nao e obrigatorio — se a prescricao em parts/exercicios ja for autoexplicativa, deixe vazio.\n- Escreva o OBJETIVO ou FOCO da sessao de hoje ("hoje o treino e continuo e em ritmo confortavel", "o foco hoje e manutencao de volume — nao force o ritmo", "a sessao de hoje trabalha a parte posterior da coxa e lombar"). Nao explique o raciocinio por tras da escolha.\n- PROIBIDO usar o formato "como voce relatou X, fiz Y" ou qualquer variante que explique a logica da prescricao. O aluno nao precisa saber o motivo tecnico — so o que fazer e com qual foco.\n- Se houver sinal de dor ou condicao de saude: registre de forma neutra e simples que foi considerado ("Seu treino de hoje levou em consideracao seu feedback sobre [condicao registrada]."), sem detalhar o que mudou ou por que. Depois disso, descreva o objetivo normalmente.\n- Nao repita o roteiro de etapas/partes em prosa — o aluno ja ve o roteiro estruturado abaixo do texto.\n- Se o treino for 1 parte so, nao mencione aquecimento/resfriamento (ja e texto padrao fixo automatico); se tiver 2+ partes com caminhada de 5+ min no inicio, esse texto padrao some e voce descreve cada bloco brevemente.',
      ...
      // 08/09: orcamento de notes ajustado — antes era 5 a 8 frases, agora 2 a 4 (notes e curto
      // por instrucao, nao apenas por limite de espaco). Rationale continua como bullets curtos.
      ...
      // 08/09: instrucao de notes para dias de forca — mesma regra dos dias de corrida (o aluno le).
      '- O campo notes de cada sessao de forca/fortalecimento segue a mesma regra dos dias de corrida: e o texto que o ALUNO le, deve ser curto (2 a 4 frases ou vazio), escreve o OBJETIVO/FOCO da sessao ("a sessao de hoje trabalha a posterior da coxa e lombar com foco em controle"), nunca uma explicacao do raciocinio por tras dos exercicios. Se houver sinal de dor: mencione de forma neutra que foi considerado, sem detalhar o que mudou. PROIBIDO o formato "como voce relatou X, fiz Y". Pode deixar vazio se os exercicios ja forem autoexplicativos.',
    ].join('\n\n');
  }
    ...
    return JSON.stringify(
      {
        ...
        autoavaliacaoDaSemanaPeloAluno: input.weeklyCheckIn
          ? (() => {
              ...
              if (ci.checkinVersion === 3) {
                // CORRECAO DE FECHAMENTO (25/09/2026, correcao 2): blocos 1-3 da v3 (avaliacao da
                // proposta, adequacao, satisfacao com execucao, exigencia da semana, resposta do
                // corpo, motivacao, interferencia esperada da rotina) sao os MESMOS valores que
                // athleteStateContext ja traz como checkin.prescriptionLiking/prescriptionSuitability/
                // executionSatisfaction/weekDemandVsNormal/bodyResponseVsNormal/postWeekMotivation/
                // expectedRoutineInterference — la' com trend/baseline/desvio/persistencia, nao so' o
                // valor da ultima semana (ver variable-registry.ts). Repeti-los aqui seria a mesma
                // informacao duas vezes. Bloco 4 (preferencia declarada + observacao livre) NAO tem
                // equivalente no Snapshot (nao sao variaveis longitudinais, sao intencao/narrativa) —
                // continua aqui como contexto complementar, unica parte do check-in v3 que so' chega
                // por este caminho.
                return {
                  versao: 3,
          ...
          : null,
        // Passo 2/6 (25/09/2026, auditoria aprovada) — ver instrucao completa no system prompt
        // (buildSystemPromptStable). Null quando o Snapshot nao pode ser gerado (nunca bloqueia a
        // geracao da semana — ver training-plans.service.ts).
        athleteStateContext: input.athleteStateContext ?? null,
        // Passo 3 (25/09/2026): trajetoria de MESES (INITIAL -> R1 -> R2 -> R3...), persistida uma
        // vez por reavaliacao concluida — nunca recalculada aqui. Complementar a
        // athleteStateContext (que descreve "agora"). Null quando o aluno nunca fez reavaliacao ou
        // a ultima esta invalidada (reaberta e ainda nao reconcluida). Nao contem score nenhum —
        // use como interpretacao de tendencia de longo prazo, nunca como regra de decisao.
        relatorioDeEvolucao: input.evolutionReport ?? null,
        hoje: input.todayDate ?? null,
        // Definido quando o aluno escolheu "Nao, a partir de amanha" no app — ver instrucao
        // "GERAR A PARTIR DE" no prompt de sistema. Null = sem restricao, incluir todos os dias.
        gerarAPartirDe: input.generateFrom ?? null,
        ...
        // So vem preenchido quando o recorde acima e >=8km — ver a regra "quente vs frio" no
        // prompt de sistema (buildSystemPromptStable). Cada item e um treino concluido nas
        // ultimas 10 semanas perto do recorde, com seu proprio autorrelato de como foi feito.
        sessoesRecentesPertoDoRecorde: (input.recentSessionsNearRecord ?? []).map((session) => ({
          distanciaKm: session.distanceKm,
          data: session.date,
          comoFoiFeito: session.pacingMode,
        })),
        // Array (correcao real 16/08 — antes so a prova mais proxima existia aqui; ver
        // TargetRacesService.activeGoals e a regra correspondente no prompt de sistema). Pode vir
        // vazio (sem nenhuma prova marcada) ou com mais de uma prova simultanea.
        metasDeProva: (input.targetRaces ?? []).map((race) => ({
          nome: race.name,
          data: race.raceDate,
          distanciaKm: race.distanceKm,
          paceAlvoSegundosPorKm: race.paceSecondsPerKm,
          // Questionario de contexto sobre a meta (10/08), 1 a 10 cada, ou null se o aluno
          // nao respondeu. Ver a regra de uso no prompt de sistema (buildSystemPromptStable).
          primeiraVezNessaDistancia: race.isFirstTimeAtDistance,
          oQuantoQuerMelhorarPerformance: race.performanceIntent,
          oQuantoEhSobreCurtirAExperiencia: race.socialIntent,
          oQuantoEhImportantePessoalmente: race.personalImportance,
          oQuantoAchaQueVaiSerDificil: race.perceivedDifficulty,
          oQuantoEstaDispostoASeDedicar: race.dedicationWillingness,
          oQuantoConseguirVaiTrazerSatisfacao: race.achievementSatisfaction,
          oQuantoConfiaQueVaiConseguir: race.confidenceLevel,
          oQuantoTemePreocupacaoComLesao: race.injuryConcern,
  ...
  private buildSingleStrengthSystemPrompt() {
    return [
      ...
      // 08/09: instrucao de notes alinhada com o padrao do agente semanal — o aluno le esse campo.
