...
// Tolerancia de validacao (nao uma decisao de conteudo — so precisao de arredondamento, ja que
// repeatCount e inteiro). Era 8 min, largo demais (quase 13% de folga num treino de 60 min) e
// permitia exatamente o tipo de desvio que o treinador reportou. A IA tem numeros continuos o
// suficiente (stepKm, easyVolumeKm) pra bater bem perto do exato, entao a margem fica pequena.
const STRUCTURE_DURATION_TOLERANCE_MIN = 3;
...
export class PrescriptionAgentService {
    ...
    try {
      ...
      if (params.sessionType === 'quality_run') {
        ...
        const estimatedMin =
          ...
          60;
        if (Math.abs(estimatedMin - params.durationMin) > STRUCTURE_DURATION_TOLERANCE_MIN) {
  ...
  private validateSessions(
    ...
    for (const session of sessions) {
      ...
      // o tempo real que essa estrutura implica bate com durationMin. Sem essa conferencia, um
      // erro de conta da propria IA (ex: repeticoes demais pro tempo disponivel) passaria batido e
      // o aluno acabaria com um treino bem mais longo/curto que o combinado — exatamente o bug que
      // ja aconteceu na pratica quando essa conta era fixa no codigo. Tolerancia pequena de
      // proposito (STRUCTURE_DURATION_TOLERANCE_MIN, so pra arredondamento de repeticoes inteiras
      // — a IA tem numeros continuos o suficiente pra bater quase exato): ja teve uma folga de 8
      // minutos aqui, larga demais pra um treino de 60 min, e o proprio treinador apontou isso.
      if (session.sessionType === 'quality_run') {
        ...
        if (Math.abs(estimatedMin - session.durationMin) > STRUCTURE_DURATION_TOLERANCE_MIN) {
          this.logger.warn(
  ...
  private buildSystemPromptStable() {
    return [
      ...
      '- ERRO GRAVE JA OBSERVADO NA PRATICA, NAO REPITA (este e diferente do erro de distancia acima — este e sobre PACE E MODALIDADE DE RECUPERACAO): quando uma diretriz da um pace explicito para um dia especifico (ex: "6 series de 1km pace de 05:00 a 05:20 intervalado com caminhada de 0,5km pace de 12:00 a 15:00"), voce ja errou na pratica ao ignorar esse pace especifico e usar, em vez dele, o paceAssessment geral da semana (intensePaceSecondsPerKm calculado pelas evidencias) para preencher intervalStructure daquele dia — o resultado foi repeatCount e fastStepKm corretos (numeros faceis de copiar), mas o pace saiu errado (nao o que a diretriz pediu) porque a regra matematica obrigatoria (ver abaixo) empurra o calculo pro paceAssessment geral por padrao. A CORRECAO: quando uma diretriz da um pace explicito pra um dia, esse pace NAO e so uma sugestao — ele SUBSTITUI o paceAssessment geral SOMENTE para o calculo daquele dia especifico (nao muda paceAssessment.intensePaceSecondsPerKm/easyPaceSecondsPerKm retornado pra semana toda, so como voce calcula a estrutura DAQUELE dia). O mesmo vale pra modalidade de recuperacao: se a diretriz diz "caminhada" (pace bem lento, tipo 12:00-15:00/km), use CAMINHADA de verdade nesse pace — nao troque por um trote mais rapido so porque acha que o aluno "aguenta melhor" um trote; a diretriz existe justamente pra sobrepor seu proprio julgamento padrao nesse dia. Aplique isso pra CADA numero que a diretriz especificar (pace do tiro, pace/distancia da recuperacao, distancia total) — todos devem aparecer EXATAMENTE como pedido na estrutura daquele dia, nao aproximados nem recalculados a partir da sua propria avaliacao de pace.',
