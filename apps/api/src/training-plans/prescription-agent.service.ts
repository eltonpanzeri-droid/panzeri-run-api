...
export class PrescriptionAgentService {
  ...
  private buildSystemPromptStable() {
    return [
      ...
      '- autoavaliacaoDaSemanaPeloAluno (quando presente) e a resposta do proprio aluno, em escala 1 a 5, a 3 perguntas sobre a semana que acabou de terminar como um TODO (nao por sessao — isso e o historicoSemanal): satisfacao com a elaboracao dos treinos, satisfacao com o proprio seguimento do plano, e motivacao declarada pra semana que voce esta gerando agora. E sinal real do aluno, nao uma formula: pese junto com o resto do contexto (historico, diretivas, execucao), nunca como unico fator decidindo sozinho. Motivacao baixa (1-2) e um bom motivo pra considerar aliviar volume/intensidade OU simplificar a comunicacao, dependendo do que o resto do contexto sugerir — nunca ignore o sinal, mas tambem nunca deixe um numero isolado sobrescrever diretriz do treinador ou dado objetivo de execucao.',
      ...
      '- athleteStateContext (quando presente) traz, por dominio (sono/recuperacao, estado fisico, estado psicologico, resposta ao treino, dor/saude, comportamento, dinamica do sistema), o estado calculado deterministicamente de cada variavel: valor atual, tendencia recente/media, baseline individual, desvio do baseline, variabilidade recente vs habitual, faixa habitual, persistencia (ha quanto tempo fora da faixa, se estiver), e a excursao mais recente com dados de retorno (recuperou o nivel? recuperou a variabilidade? sao coisas DIFERENTES — um aluno pode voltar pro nivel normal e continuar bem mais instavel do que era). Cada variavel tambem informa evidence.n (quantas observacoes sustentam aquele calculo) — trate uma excursao calculada sobre 3 observacoes com muito mais cautela do que um padrao sustentado por dezenas, mas nao existe um numero magico de corte: use julgamento, do jeito que voce pesaria um relato isolado vs um padrao repetido de um aluno real.',
      '- REGRAS DURAS sobre athleteStateContext: (1) NUNCA trate uma variavel isolada como determinante — fadiga fisica alta pode coexistir com semana pesada de proposito, boa aderencia, sono bom, resposta boa ao estimulo, ausencia de dor; sono pior isoladamente NAO implica automaticamente aliviar o treino; RPE alto isoladamente NAO implica reduzir volume. Olhe a CONFIGURACAO do sistema inteiro, nao uma variavel sozinha. (2) O MESMO valor atual pode significar coisas diferentes dependendo do baseline/trajetoria daquele aluno especifico — fadiga=4 com baseline habitual=2 e persistente e MUITO diferente de fadiga=4 com baseline=4 e dentro do padrao normal dele; preste atencao em currentlyOutsideHabitualRange e na trend, nao so no valor bruto. (3) Cada escala mantem a semantica original do VariableRegistry (constructLabel/direction) — 5 sempre significa "mais daquele construto especifico", nunca "bom" universalmente; confira o constructLabel de cada variavel antes de interpretar. (4) systemDynamics.variablesCurrentlyOutsideHabitualRange/ongoingExcursions/recentlyRecoveredExcursions sao observacoes descritivas, nao diagnostico nem recomendacao pronta — a interpretacao de causa/efeito entre variaveis (ex: treino->fadiga->sono->recuperacao, ou estresse->sono->execucao) e sua, baseada no contexto real, nunca uma regra fixa codificada. (5) Onde este campo e redundante com dados que voce ja recebe em outro formato (ex: os blocos numericos de autoavaliacaoDaSemanaPeloAluno, cujos MESMOS valores aparecem aqui com historico/tendencia), prefira este campo por ter mais contexto — mas ambos continuam presentes nesta fase de transicao.',
