...
document.querySelectorAll('[data-checkout]').forEach(link=>{link.href=checkout.href;link.addEventListener('click',()=>{trackEvent(link.dataset.track);trackEvent('checkout_start',{destination:'student_app'});if(typeof window.fbq==='function')window.fbq('track','InitiateCheckout')})});
